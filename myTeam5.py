...
class DynamicAgent(CaptureAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    # This creates a half grid layout of just our side of the map. This is used to find "Safe paths" between 2 points on our side of our map. Safe paths are those that only go through our side of the map. If we don't do this, then the shortest path between 2 squares might go through enemy territory, even if the 2 points are on our side of the map. If we want to be safe, then we don't want to go to enemy territory. 
    fullgrid = gameState.getWalls()
    halfGrid = capture.halfGrid(fullgrid, red=self.red)

    # state_copy = gameState.deepcopy()
    layout_text = str(gameState.data.layout).split("\n")
    half_layout_text = []
    for line in layout_text:
      #print(line)
      wallBuffer = "".join(["%" for i in range(len(line)//2)])
      half_layout_text.append(line[:len(line)//2] + wallBuffer) if self.red else half_layout_text.append(wallBuffer + line[len(line)//2:])

    
    print("\n".join(half_layout_text))

    half_layout = layout.Layout(half_layout_text)

    #print(half_layout)
    
    self.half_dist = distanceCalculator.Distancer(half_layout)

    # comment this out to forgo maze distance computation and use manhattan distances
    self.half_dist.getMazeDistances()
  ...
  def goToRandom(self, actions, gameState):
    ...
    for i in range(len(actions)*4):
      for action in actions:
        ...
        if self.posIsSafe(gameState.getWalls(), successorPos) and self.half_dist.getDistance(successorPos, self.randomStartPos)<1000 and self.half_dist.getDistance(successorPos, self.randomStartPos) < self.half_dist.getDistance(agentPos, self.randomStartPos):
          if successorPos == self.randomStartPos:
            self.randomStartPos = None
    ...
    acceptableActions = []
    for action in actions:
      if self.posIsSafe(gameState.getWalls(), self.actionPos(gameState, action)[1]):
  ...
  def chooseAction(self, gameState):
    ...
    if self.randomStartPos is not None:
        return self.goToRandom(actions, gameState)
    ...
    if(self.holdingFood < 2):     
      ...
      for opp in oppTheirSide:
          if self.distancer.getDistance(agentPos, opp[1]) < ((minFoodDist+distFromFoodToHome)*1.6):
            goHome = True

      if self.posIsSafe(walls, agentPos) and oppTheirSide:
        for opp in oppTheirSide:
          if self.getMazeDistance(agentPos, opp[1]) < 7:
            return self.goToRandom(actions, gameState)
    ...
    if(goHome or self.holdingFood):
      ...
      if not oppTheirSide: # If we are not being chased by a ghost, we can go home easily, just take the shortest path home
        ...
        # print(gameState.data)
        safeSpaces = halfList(l=gridList, grid=walls, red=self.red)
        oppHalf = halfList(l=gridList, grid=walls, red=not self.red)
        # print(safeSpaces)
        best_score=None

        edgeIndex = walls.width//2-1 if self.red else walls.width//2
        safeStrip = [x for x in safeSpaces if x[1]==edgeIndex]

        currentRunScore = 1
        for opp in oppTheirSide:
          currentRunScore *= self.getMazeDistance(opp[1], agentPos)
        currentRunScore = currentRunScore ** (1/len(oppTheirSide))

        '''for safe_square in safeStrip:
          # Maximize distance from opponents by multiplying distance 
          minDist = None
          bestSquare = None
          for temp_square in gridList:
            distToSquare = self.getMazeDistance(agentPos, temp_square)
            distToHome = self.getMazeDistance(safe_square, )

            runScore = 1
            for opp in oppTheirSide:
              runScore *= self.getMazeDistance(opp[1], temp_square)
            runScore = runScore ** (1/len(oppTheirSide))

            if runScore < currentRunScore:
              if minDist is None or distToSquare + distToHome < minDist:
                minDist = distToSquare + distToHome
                bestSquare = temp_square
        
        for action in actions:
          successor = self.getSuccessor(gameState, action)
          successorPos = successor.getAgentPosition(self.index)
          if(self.getMazeDistance(agentPos, bestSquare) > self.getMazeDistance(successorPos, bestSquare)):
            return action
...
class DefenseAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    if len(oppPacmans) > 0:
      ...
      for action in actions:
        ...
        if self.getMazeDistance(successorPos, self.chasePos) < self.getMazeDistance(agentPos, self.chasePos) and self.posIsSafe(grid, successorPos):
          ...
          print("chasing pacman cuz it ate food")
          return action
    ...
    acceptableActions = []
    for action in actions:
      if self.posIsSafe(grid, self.actionPos(gameState, action)[1]):
        acceptableActions += action
  ...
  def goToRandom(self, actions, gameState):
    ...
    for action in actions:
      ...
      if self.posIsSafe(gameState.getWalls(), successorPos) and self.half_dist.getDistance(successorPos, self.randomDefPos)<1000 and self.half_dist.getDistance(successorPos, self.randomDefPos) < self.half_dist.getDistance(agentPos, self.randomDefPos):
        if successorPos == self.randomDefPos:
          self.randomDefPos = None
        
        print('going to random defense:', self.randomDefPos, agentPos)
        return action
  ...
  def actionPos(self, gameState, action):
    successor = self.getSuccessor(gameState, action)
    successorPos = successor.getAgentPosition(self.index)
    return (action, successorPos)

  def posIsSafe(self, grid, pos):
    if self.red:
      return pos[0] < grid.width//2-1
    else:
      return pos[0] >= grid.width//2
